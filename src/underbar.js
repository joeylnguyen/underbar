(function() {
  ...
  _.each = function(collection, iterator) {
    // Iterate when collection is an array
    if (Array.isArray(collection) === true) {
      for (var i = 0; i < collection.length; i++) {
        iterator(collection[i], i, collection);
      }
    // Iterate when collection is an object
    } else {
  ...
  // Apply iterator to current number
  // Push result to result variable
  ...
  _.defaults = function(obj) {
    _.each(arguments, function(extenderObj) { // loops through the arguments 
      _.each(extenderObj, function(item, key) { // applies the each function and loops through each of the individual extender objects and adds the properties to the main object if it doesn't exist
  ...
  // http://mdn.io/Array.prototype.slice
   _.shuffle = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      resultsArray.push(arrayCopy[random]);
